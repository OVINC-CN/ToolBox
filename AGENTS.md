...
- 本项目是部署到 Cloudflare Pages 的 Vite + React + TypeScript 多页面工具集。
...
- `public/` 存放直接复制到产物的静态文件；`scripts/publish-pages.mjs` 将 Vite 生成的页面入口移到公开路径，并生成 Cloudflare Pages 的 `_redirects`。
- 鹈鹕原页面使用根绝对路径；构建产物通过 `_redirects` 指向 `dist/bike/` 的单份原文件，不在 `dist` 根目录复制版本 ID 目录。
